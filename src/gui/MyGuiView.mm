...
-(IBAction)hide:(id)sender{
  ...
  NSLog(@"host: %@", host);
  if (host != nil){
    string myhost = ofxNSStringToString(host);
    myApp->changeHost(myhost);
  }
}
...
-(IBAction)changeHost:(id)sender{
	...
	UITextField * tfield = sender;
  host = [[tfield text] copy];
	printf("textfield value is - %s\n", ofxNSStringToString([tfield text]).c_str());
}
